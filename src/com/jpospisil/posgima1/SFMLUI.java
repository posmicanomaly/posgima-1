...
public class SFMLUI {
	...
	public SFMLUI(SFMLRenderWindow window, Game game) {
		...
		sideView = new View(window.getRenderWindow().getDefaultView()
				...
				.getSize());
		bottomView = new View(window.getRenderWindow().getDefaultView()
				.getCenter(), window.getRenderWindow().getDefaultView()
				.getSize());
	...
	public void drawBottomUI() {
		...
		Text text = new Text(madeMessage, GameConstants.FONTS.getDefaultFont(),
				GameConstants.BOTTOM_UI_FONT_SIZE);
		text.setPosition(
				(int) (GameConstants.WIDTH * GameConstants.VIEWPORT_TOP_X),
				(int) (GameConstants.HEIGHT * GameConstants.VIEWPORT_BOTTOM_Y));
