...
public class SFMLASCIIRender {
	...
	private void getGlyphs(GameMap gameMap, Player player)
	{
		...
		int renderedCount = 0;
		ArrayList<Tile> visibleTiles = gameMap.getTilesInRange(player, GameConstants.VISIBLE_LEFT, GameConstants.VISIBLE_RIGHT,
		...
		for(int i = 0; i < visibleTiles.size(); i++)
		{
			...
			//tileGlyphs.add(text);
			window.getRenderWindow().draw(text);
			if(tile.getDug() && tile.getType() != "road" && tile.getType() != "house")
		...
		//	System.out.print("\nRendered: " + renderedCount + " t? " + gameMap.getTileArray().size());
		//
	...
	{
		text = new Text("@", GameConstants.FONTS.getDefaultFont(), GameConstants.DEFAULT_FONT_SIZE);
		...
		{
			window.getWorldView().setCenter((float) (tile.getX()- (GameConstants.WIDTH * .01 + 1)), tile.getY());
		}
		else //16:10 master race
	...
	public void renderWin(GameMap gameMap)
	{
		for(Tile tile : gameMap.getTileArray())
		{
			for(Item item : tile.getItems())
			{
				if(item.getName() == "win")
				{
					Text text = new Text("X", GameConstants.FONTS.getDefaultFont(), GameConstants.DEFAULT_FONT_SIZE);
					text.setColor(GameConstants.PLAYER_COLOR);
					text.setPosition(tile.getX(), tile.getY());
					window.getRenderWindow().draw(text);
				}
